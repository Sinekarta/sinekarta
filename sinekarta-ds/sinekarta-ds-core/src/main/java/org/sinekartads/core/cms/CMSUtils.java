...
class CMSUtils
{
	...
	static List getCertificatesFromStore(CertStore certStore)
		...
		try {
			for (Iterator it = certStore.getCertificates(null).iterator(); it
					.hasNext();) {
				X509Certificate c = (X509Certificate) it.next();

				certs.add(X509CertificateStructure.getInstance(ASN1Object
						.fromByteArray(c.getEncoded())));
			}
	...
	static List getCertificatesFromStore(Store certStore) throws CMSException {
		...
		try {
			for (Iterator it = certStore.getMatches(null).iterator(); it
					.hasNext();) {
	...
	static List getAttributeCertificatesFromStore(Store attrStore)
		...
		try {
			for (Iterator it = attrStore.getMatches(null).iterator(); it
					.hasNext();) {
				X509AttributeCertificateHolder attrCert = (X509AttributeCertificateHolder) it
						.next();

				certs.add(new DERTaggedObject(false, 2, attrCert
						.toASN1Structure()));
			}
	...
	static List getCRLsFromStore(CertStore certStore)
		...
		try {
			for (Iterator it = certStore.getCRLs(null).iterator(); it.hasNext();) {
				...
				crls.add(CertificateList.getInstance(ASN1Object.fromByteArray(c
						.getEncoded())));
			}
	...
	static List getCRLsFromStore(Store crlStore) throws CMSException {
		...
		try {
			for (Iterator it = crlStore.getMatches(null).iterator(); it
					.hasNext();) {
	...
	static OutputStream createBEROctetOutputStream(OutputStream s, int tagNo,
			boolean isExplicit, int bufferSize) throws IOException {
		BEROctetStringGenerator octGen = new BEROctetStringGenerator(s, tagNo,
				isExplicit);
	...
	static IssuerAndSerialNumber getIssuerAndSerialNumber(X509Certificate cert) {
		...
		return new IssuerAndSerialNumber(tbsCert.getIssuer(), tbsCert
	...
	static InputStream attachDigestsToInputStream(Collection digests,
			InputStream s) {
	...
	static OutputStream attachDigestsToOutputStream(Collection digests,
			OutputStream s) {
	...
	static OutputStream attachSignersToOutputStream(Collection signers,
			OutputStream s) {
		OutputStream result = s;
		...
		while (it.hasNext()) {
			...
			result = getSafeTeeOutputStream(result,
					signerGen.getCalculatingOutputStream());
		}
		return result;
	}
	...
	static class CMSStreamException extends IOException {
