...
/**
 * Patched by JENIA Software to adapt itext to bcprov-jdk16. <br/>
...
*/
    ...
    public byte[] getAuthenticatedAttributeBytes(byte secondDigest[], Calendar signingTime, byte[] ocsp, Collection<byte[]> crlBytes, CryptoStandard sigtype) {
        try {
            return getAuthenticatedAttributeSet(secondDigest, signingTime, ocsp, crlBytes, sigtype).getDEREncoded();
        }
        catch (Exception e) {
            throw new ExceptionConverter(e);
        }
    ...
    public boolean verify() throws GeneralSecurityException {
        ...
        if (isTsp) {
            ...
            byte[] imphashed = info.getMessageImprintDigest();
