		...
		try {
			...
			try {
				chainSignatureDTO = (SignatureDTO) converter.fromSignatureInfo ( 
						(SignatureInfo < SignCategory, SignDisposition.CMS, 
			...
			try {
				digestSignature = (DigestSignature < SignCategory, SignDisposition.CMS, 
					     							 VerifyResult, CMSSignatureInfo >) converter.toSignatureInfo(digestSignatureDTO);
			} catch(Exception e) {
			...
			try {
				finalizedSignature = (FinalizedSignature < SignCategory, SignDisposition.CMS, 
					     								   VerifyResult, CMSSignatureInfo >) converter.toSignatureInfo ( finalizedSignatureDTO );
			} catch(Exception e) {
				tracer.error("unable to obtain the digestSignature from the DTO", e);
				throw e;
			}
			...
			try {
				finalizedSignatureDTO = (SignatureDTO) converter.fromSignatureInfo ( 
						(SignatureInfo < SignCategory, SignDisposition.CMS, 
									  	 VerifyResult, CMSSignatureInfo >) finalizedSignature );
			} catch(Exception e) {
				tracer.error("unable to convert the chainSignature to a DTO", e);
				throw e;
			}
