...
<script type="text/javascript">//<![CDATA[
	...
	(function()
	{
		...
		YAHOO.lang.extend ( Alfresco.skds, 
			...
			prepareForm: function skds_prepareForm ( targetForm ) {
				this.callFormOperation ( targetForm, 'prepare' );
			},
			...
			processForm: function skds_processForm ( targetForm ) {
				...
				if ( targetForm !== this.wizardData.currentForm ) {
					prepareForm ( this.wizardData.currentForm );
				}
			},
		    ...
		    updateWizardDataView: function skds_updateWizardDataView ( ) {
		    	var prettyJSON = this.formatJSON ( this.wizardData );
			...
			onBackClick: function skds_onBackClick(e) 
			{
				...
				var prevForm = this.currentForm;
				for ( i=0; i<this.wizardForms.length; i++ ) {
					if ( this.wizardForms[i+1] === this.currentForm ) {
			...
			onUndoClick: function skds_onUndoClick(e) 
			{
				...
				this.refresh ( this.backupDataJSON, this.currentForm );
			},
			
			onNextClick: function skds_onNextClick(e) 
			{
				var callerForm = this.wizardData.currentForm;
				// processForm() will refresh the whole form and change the value of this.currentForm
				...
				if ( callerForm !== this.currentForm ) {
					this.prepareForm ( callerForm );
				}
			},
			...
			refresh: function skds_refresh ( wizardDataJSON, targetForm ) 
			{
		    	...
		    	/*this.info ( 'refresh', 
						'targetForm: ' + targetForm + '\n' +
					   	'currentForm: ' + this.currentForm + '\n' +
					   	'wizardData.currentForm: ' + this.wizardData.currentForm );*/
		    	
				// Extract the currentForm from the wizardData if missing
				if ( this.currentForm === undefined ) {
					this.currentForm = this.wizardData.currentForm;
				}
				
				// Update the displayed form if changed
				//if ( this.currentForm !== targetForm ) {
			    	if ( targetForm !== undefined ) {
			    		// Update the currentForm with the targetForm if provided
						this.currentForm = targetForm;
					} else {
						// Extract the correntForm from the wizardData otherwise
						this.currentForm = this.wizardData.currentForm;
					}
				...
				ksAliasesSelect.options[0] = new Option('${msg("select.dummy")}', '', selected, false);
	...
	function getAlias(){
				...
				document.sinekartaApplet.setPin(pinInputObj.value);			
				var alias = document.sinekartaApplet.getAlias(document.sinekartaApplet.getPin());
				var aliasArray = JSON.parse(alias);
				var aliasSelect = document.getElementById("${htmlid}-scUserAlias");
				ClearOptionsAlt("${htmlid}-scUserAlias");
			...
			function getCertificationChain() {
				...
				var pinObj = document.getElementById("${htmlid}-scPin");
				var aliasObj = document.getElementById("${htmlid}-scUserAlias");
				if(aliasObj.value != '') {
					var chain = document.sinekartaApplet.getCertificationChain(pinObj.value, aliasObj.value);
				}
			...
			function ClearOptionsAlt(selectId)
			{
